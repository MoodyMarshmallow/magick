// Stores thread metadata and projected thread snapshots.

import { Context, Effect, Layer } from "effect";

import type {
  ThreadRecord,
  ThreadSummary,
  ThreadViewModel,
} from "@magick/contracts/chat";
import { PersistenceError } from "../effect/errors";
import type { DatabaseClient } from "./database";

export interface ThreadRepositoryService {
  readonly create: (
    thread: ThreadRecord,
  ) => Effect.Effect<void, PersistenceError>;
  readonly get: (
    threadId: string,
  ) => Effect.Effect<ThreadRecord | null, PersistenceError>;
  readonly listByWorkspace: (
    workspaceId: string,
  ) => Effect.Effect<readonly ThreadSummary[], PersistenceError>;
  readonly getSnapshot: (
    threadId: string,
  ) => Effect.Effect<ThreadViewModel | null, PersistenceError>;
  readonly saveSnapshot: (
    threadId: string,
    summary: ThreadSummary,
    thread: ThreadViewModel,
  ) => Effect.Effect<void, PersistenceError>;
}

export const ThreadRepository = Context.GenericTag<ThreadRepositoryService>(
  "@magick/ThreadRepository",
);

export const makeThreadRepositoryLayer = (database: DatabaseClient) =>
  Layer.succeed(ThreadRepository, {
    create: (thread) =>
      Effect.try({
        try: () => {
          database
            .prepare(
              `
                INSERT INTO threads (
                  id,
                  workspace_id,
                  provider_key,
                  provider_session_id,
                  title,
                  created_at,
                  updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
              `,
            )
            .run(
              thread.id,
              thread.workspaceId,
              thread.providerKey,
              thread.providerSessionId,
              thread.title,
              thread.createdAt,
              thread.updatedAt,
            );
        },
        catch: (error) =>
          new PersistenceError({
            operation: "thread_repository.create",
            detail: error instanceof Error ? error.message : String(error),
          }),
      }),
    get: (threadId) =>
      Effect.try({
        try: () => {
          const row = database
            .prepare(
              `
                SELECT id, workspace_id, provider_key, provider_session_id, title, created_at, updated_at
                FROM threads
                WHERE id = ?
              `,
            )
            .get(threadId) as
            | {
                id: string;
                workspace_id: string;
                provider_key: string;
                provider_session_id: string;
                title: string;
                created_at: string;
                updated_at: string;
              }
            | undefined;

          return row
            ? {
                id: row.id,
                workspaceId: row.workspace_id,
                providerKey: row.provider_key,
                providerSessionId: row.provider_session_id,
                title: row.title,
                createdAt: row.created_at,
                updatedAt: row.updated_at,
              }
            : null;
        },
        catch: (error) =>
          new PersistenceError({
            operation: "thread_repository.get",
            detail: error instanceof Error ? error.message : String(error),
          }),
      }),
    listByWorkspace: (workspaceId) =>
      Effect.try({
        try: () => {
          const rows = database
            .prepare(
              `
                SELECT snapshots.summary_json AS summary_json
                FROM thread_snapshots AS snapshots
                INNER JOIN threads ON threads.id = snapshots.thread_id
                WHERE threads.workspace_id = ?
                ORDER BY snapshots.updated_at DESC
              `,
            )
            .all(workspaceId) as { summary_json: string }[];

          return rows.map(
            (row) => JSON.parse(row.summary_json) as ThreadSummary,
          );
        },
        catch: (error) =>
          new PersistenceError({
            operation: "thread_repository.listByWorkspace",
            detail: error instanceof Error ? error.message : String(error),
          }),
      }),
    getSnapshot: (threadId) =>
      Effect.try({
        try: () => {
          const row = database
            .prepare(
              "SELECT thread_json FROM thread_snapshots WHERE thread_id = ?",
            )
            .get(threadId) as { thread_json: string } | undefined;

          return row ? (JSON.parse(row.thread_json) as ThreadViewModel) : null;
        },
        catch: (error) =>
          new PersistenceError({
            operation: "thread_repository.getSnapshot",
            detail: error instanceof Error ? error.message : String(error),
          }),
      }),
    saveSnapshot: (threadId, summary, thread) =>
      Effect.try({
        try: () => {
          database
            .prepare(
              `
                INSERT INTO thread_snapshots (thread_id, summary_json, thread_json, updated_at)
                VALUES (?, ?, ?, ?)
                ON CONFLICT(thread_id) DO UPDATE SET
                  summary_json = excluded.summary_json,
                  thread_json = excluded.thread_json,
                  updated_at = excluded.updated_at
              `,
            )
            .run(
              threadId,
              JSON.stringify(summary),
              JSON.stringify(thread),
              thread.updatedAt,
            );

          database
            .prepare("UPDATE threads SET updated_at = ? WHERE id = ?")
            .run(thread.updatedAt, threadId);
        },
        catch: (error) =>
          new PersistenceError({
            operation: "thread_repository.saveSnapshot",
            detail: error instanceof Error ? error.message : String(error),
          }),
      }),
  });
