...
export class ProviderAuthRepositoryClient {
  ...
  upsert(record: ProviderAuthRecord): Effect.Effect<void, PersistenceError> {
    return Effect.try({
      try: () => {
        this.#database
          .prepare(
  ...
  ): Effect.Effect<ProviderAuthRecord | null, PersistenceError> {
    return Effect.try({
      try: () => {
  ...
  delete(providerKey: ProviderKey): Effect.Effect<void, PersistenceError> {
    return Effect.try({
      try: () => {
        this.#database
...
export interface ProviderAuthRepositoryService {
  readonly upsert: (
...
export const ProviderAuthRepository =
  Context.GenericTag<ProviderAuthRepositoryService>(
    "@magick/ProviderAuthRepository",
  );

export const makeProviderAuthRepositoryLayer = (database: DatabaseClient) => {
  const client = new ProviderAuthRepositoryClient(database);
  return Layer.succeed(ProviderAuthRepository, client);
};
